...
function Footer({ sections, logo, richText, socialMedia, isDesktop }: ReturnType<typeof loader>) {
  const Divider = () => <div class="h-px w-full bg-white" />
  return (
    <div class="bg-[#414042]">
      <footer class="container flex flex-col py-10 gap-20 text-white">
        <div class={`flex flex-wrap justify-start ${isDesktop ? "gap-6" : "flex-col"}`}>
          {sections.map((props) => <Links {...props} isDesktop={isDesktop} />)}
        </div>
        {isDesktop && <Divider />}
        <div class="flex justify-between items-center py-8 flex-wrap gap-16">
          {isDesktop && <Image src={logo} width={212} height={63} decoding={"async"} loading={"lazy"} />}
          {isDesktop && <div class="max-w-[312px] lg:max-w-[478px]" dangerouslySetInnerHTML={{ __html: richText }} />}
          <Social {...socialMedia} />
          {!isDesktop && <div class="flex flex-col gap-7">
            <Image src={logo} width={212} height={63} decoding={"async"} loading={"lazy"} />
            <div class="max-w-[312px] lg:max-w-[478px]" dangerouslySetInnerHTML={{ __html: richText }} />
          </div>}
        </div>
      </footer>
    </div>

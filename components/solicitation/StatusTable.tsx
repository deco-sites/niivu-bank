...
export const loader = (props: Props, _req: Request, ctx: AppContext) => {
  ...
  return {
    ...props,
    solicitations: props.solicitations as DataObjectSoliciation[],
    isDesktop: ctx.device === "desktop",
  };
};

function StatusTable(
  { solicitations, status, statusMessageDefault, isDesktop }: ReturnType<
    typeof loader
  >,
) {
  return (
      ...
      <div class="relative container flex flex-col gap-5 lg:gap-32 pb-48 pt-5 lg:pt-6">
        ...
        <div class="flex flex-col">
          ...
          {solicitations?.map((solicitation, index) => (
            <div class="collapse rounded-none group/form">
              ...
              <label htmlFor={`${index}`}>
                <div class="collapse-title min-h-0 p-0 rounded-none">
                  <div class="flex justify-start items-center gap-4 leading-4 text-[#111214] border-b border-[#E0E0E0] h-10 bg-transparent text-sm py-3 px-2 max-md:gap-2 max-md:text-[11px] group-has-[input:checked]/form:border-none">
                    ...
                    <p class="max-w-80 w-full h-6">
                      {solicitation.cnpj ?? solicitation.cpf}
                    </p>
                    <div class="max-w-80 w-full h-6">
                      <div
                        class="max-w-40 h-6 rounded-full gap-2 p-2 bg-[#F0F2F4] flex justify-start items-center"
                        style={{
                          color: status.find(({ statusType }) =>
                            statusType === solicitation.status
                          )?.color,
                        }}
                      >
                        <div
                          class="rounded-full w-2 h-2 flex-shrink-0 mt-1"
                          style={{
                            backgroundColor: status.find(({ statusType }) =>
                              statusType === solicitation.status
                            )?.color ?? "black",
                          }}
                        />
                    ...
                    <label class="swap group-has-[input:checked]/form:swap-active swap-rotate ml-auto -z-10">
                      <Icon
                        ...
                        width={24}
                        height={24}
                        class="swap-off text-[#888D95]"
                      />
                      <Icon
                        id="ChevronDown"
                        width={24}
                        height={24}
                        class="swap-on text-[#888D95]"
                      />
                    </label>
                  </div>
                </div>
              ...
              <div class="collapse-content flex justify-center items-center">
                <div class="py-4">
                  <StatusBar
                    solicitation={{
                      ...solicitation,
                      status: solicitation.status === STATUS_ENUM_RISK3_FAILED
                        ? STATUS_ENUM_ACCOUNT_OPENING
                        : solicitation.status,
                    }}
                    statusMessageDefault={statusMessageDefault}
                    status={status}
                    isDesktop={isDesktop}
                  />
                </div>
